...
const bundler = new Bundler('./index.html', { publicUrl: '/caba/sign-up' });

app.get('/caba/config.json', (req, res) => {
  res.type('json');
...
Promise.all([
  ...
  console.log(`Server running at ${chalk.bold(chalk.cyan('http://localhost:1234'))}`);
});
