...
const When : React.FC<WhenProps> = ({ children, pred }) => {
  return (
    pred() && <React.Fragment>{ children }</React.Fragment>
  );
}
...
const MeetTheTeam : React.FC = () => {
  ...
  const [teamResults, setTeamResults] = React.useState<TeamResults>(EMPTY_TEAM_RESULTS);
  ...
  React.useEffect(() => {
    const getTeamData = async () => {
      try {
        ...
        const hasMore = (response.meta.offset * response.meta.limit) < response.meta.count;
        setTeamResults({ 
  ...
  return (
    <React.Fragment>
      <Header />
      <InfoPage title="Kooth in Association with CABA | Meet the team">
        <h1>Meet the Team</h1>
        { teamResults.loading && <div className="loading-box"><span className="spinner"></span>Loading</div> }
        { errored && <span>Sorry, we couldn't load the team page right now. Try refreshing the page in a few moments.</span> }
        <ul className="team-list">
            { teamResults.team.map((result : TeamMember, i : number) => <TeamMemberList key={ i } person={ result } />) }
        </ul>
        { teamResults.more && <a href="#" className="paginate" onClick={ next }>Next</a> }
        { (page > 1) && <a href="#" className="paginate" onClick={ previous }>Back</a> }
      </InfoPage>
      <Footer />
    </React.Fragment>
  );
